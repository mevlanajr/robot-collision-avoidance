import math
import matplotlib.pyplot as plt
import numpy as np
from coppeliasim_zmqremoteapi_client import RemoteAPIClient

# 1) Connect & start
client = RemoteAPIClient()
sim    = client.require('sim')
simIK  = client.require('simIK')
# --- MODIFIED: Load the UR5 scene ---
sim.loadScene('/home/mevlanajr/Projects/robot-collision-avoidance/models/cbf_6DOF.ttt') # Assumes your scene is named this

# 2) Get handles
# --- MODIFIED: Get handles for all 6 joints ---
joint_names = [
    '/UR5/base_joint', '/UR5/shoulder_joint', '/UR5/elbow_joint',
    '/UR5/wrist_1_joint', '/UR5/wrist_2_joint', '/UR5/wrist_3_joint'
]
joint_handles = [sim.getObject(name) for name in joint_names]
# ---------------------------------------------
tip      = sim.getObject('/UR5/tip')
base     = sim.getObject('/UR5')
target   = sim.getObject('/target')
obstacle = sim.getObject('/obstacle')

# --- MODIFIED: Set all 6 joints to KINEMATIC mode ---
for handle in joint_handles:
    sim.setJointMode(handle, sim.jointmode_kinematic)
# --------------------------------------------------

# 3) --- SWITCH TO SYNCHRONOUS MODE ---
sim.setStepping(True)
sim.startSimulation()

# 4) --- TRAJECTORY & CONTROLLER PARAMETERS ---
# Get initial positions to calculate workspace
initial_tip_pos = np.array(sim.getObjectPosition(tip, -1))
base_pos        = np.array(sim.getObjectPosition(base, -1))

# --- MODIFIED: A simpler way to define a safe trajectory for 6-DOF ---
# We will create a trajectory in front of the robot.
radius            = 0.3  # m
angular_frequency = 0.5  # rad/s
# Center the circle 0.5m in front of the robot's base
center_xyz        = np.array([base_pos[0] + 0.5, base_pos[1], base_pos[2] + 0.4])
# ---------------------------------------------------------------------

# --- PD Controller gains ---
gamma_p = 6.0
gamma_d = 0.0
lamda   = 0.01
# -------------------------------------------------------

# --- CBF parameters ---
d_min = 0.2  # minimum safe distance (m)
k_h   = 2.8   # class-K gain: alpha(h) = k_h * h
# -------------------------------------------------------

# 5) --- Build IK group ---
# --- MODIFIED: Use X, Y, and Z constraints for 3D space ---
ikEnv   = simIK.createEnvironment()
ikGroup = simIK.createGroup(ikEnv)
simIK.addElementFromScene(ikEnv, ikGroup, base, tip, target,
                          simIK.constraint_x | simIK.constraint_y | simIK.constraint_z)
# ---------------------------------------------------------

# 6) --- Data logs for plotting ---
times = []
error_norm = []
ee_path_log = []
desired_path_log = []
barrier_h_log = []
obstacle_path_log = [] # <<-- NEW: Log for the obstacle's path

# 7) --- SYNCHRONOUS JACOBIAN CONTROL LOOP ---
print("Starting 6-DOF controller...")
duration       = 40
start_sim_time = sim.getSimulationTime()
dt             = sim.getSimulationTimeStep()

# --- Initialize variables ---
last_ee_pos = initial_tip_pos
# --- NEW: Get initial position of the dynamic obstacle ---
dynamic_obs_initial_pos = np.array(sim.getObjectPosition(obstacle, -1))
# ----------------------------------------------------------

while sim.getSimulationTime() - start_sim_time < duration:
    t = sim.getSimulationTime() - start_sim_time

    # --- NEW: DYNAMIC OBSTACLE UPDATE ---
    # Make the obstacle move up and down in a sine wave
    obs_amplitude = 0.15 # m
    obs_frequency = 0.25 # Hz
    new_obs_z = dynamic_obs_initial_pos[2] + obs_amplitude * math.sin(2 * math.pi * obs_frequency * t)
    new_obs_pos = [dynamic_obs_initial_pos[0], dynamic_obs_initial_pos[1], new_obs_z]
    sim.setObjectPosition(obstacle, -1, new_obs_pos)
    # ------------------------------------

    # --- MODIFIED: 3D Trajectory Generation ---
    # Circle in the YZ plane
    ang = angular_frequency * t
    desired_xyz = center_xyz + np.array([0,
                                         radius * math.cos(ang),
                                         radius * math.sin(ang)])
    desired_velocity_xyz = np.array([0,
                                     -radius * angular_frequency * math.sin(ang),
                                      radius * angular_frequency * math.cos(ang)])
    
    sim.setObjectPosition(target, -1, desired_xyz.tolist())
    # ------------------------------------------

    # --- Control Calculations ---
    # MODIFIED: Get 3D position
    ee_pos = np.array(sim.getObjectPosition(tip, -1))
    
    # MODIFIED: Compute 3x6 Jacobian
    simIK.syncFromSim(ikEnv, [ikGroup])
    jac_flat, _ = simIK.computeGroupJacobian(ikEnv, ikGroup)
    Jxyz = np.array(jac_flat).reshape((3, 6))

    # --- PD Control Law (now in 3D) ---
    error         = desired_xyz - ee_pos
    # Handle dt=0 case at the first step
    if dt > 0:
        ee_velocity   = (ee_pos - last_ee_pos) / dt
    else:
        ee_velocity = np.zeros(3)
    error_dot     = desired_velocity_xyz - ee_velocity
    v_cmd         = desired_velocity_xyz + gamma_p * error + gamma_d * error_dot
    
    # MODIFIED: Solve for 6 joint velocities
    JTJ = Jxyz.T @ Jxyz + lamda * np.eye(6)
    # CORRECTED: Use Jxyz instead of Jxy
    dq  = np.linalg.solve(JTJ, Jxyz.T @ v_cmd)

    # === CBF filter (now in 3D) ===
    obs_pos = np.array(sim.getObjectPosition(obstacle, -1))
    delta_p = ee_pos - obs_pos
    h_val   = delta_p.dot(delta_p) - d_min**2
    grad_h  = 2.0 * delta_p           # 1x3
    J_h     = grad_h @ Jxyz           # 1x6
    residual = J_h.dot(dq) + k_h * h_val

    if residual < 0.0:
        norm2 = J_h.dot(J_h)
        if norm2 > 1e-6:
            dq    = dq - (residual / norm2) * J_h
    # === end CBF filter ===

    # --- MODIFIED: Command all 6 joints ---
    current_q = np.array([sim.getJointPosition(j) for j in joint_handles])
    q_desired  = current_q + dq * dt
    
    for i in range(6):
        sim.setJointPosition(joint_handles[i], q_desired[i])
    # ------------------------------------

    # --- Logging and Stepping ---
    times.append(t + start_sim_time)
    desired_path_log.append(desired_xyz)
    ee_path_log.append(ee_pos)
    error_norm.append(np.linalg.norm(error))
    barrier_h_log.append(h_val)
    obstacle_path_log.append(new_obs_pos) # <<-- NEW: Log the obstacle's position
    
    last_ee_pos = ee_pos
    sim.step()

# --- Get obstacle info for plotting ---
obs_pos_final = sim.getObjectPosition(obstacle, -1)
size, _ = sim.getShapeBB(obstacle)
obs_radius = size[0] / 2
# --------------------------------------------------------------------

# 8) Stop simulation
sim.stopSimulation()
print("Controller finished.")

# 9) Plotting
if times:
    # --- NEW: Helper function to draw a 3D sphere ---
    def plot_sphere(ax, center, radius, color, alpha=0.2, label=None):
        # --- MODIFIED: Reduced resolution for faster plotting ---
        u = np.linspace(0, 2 * np.pi, 20) # Was 100
        v = np.linspace(0, np.pi, 20)   # Was 100
        # ----------------------------------------------------
        x = radius * np.outer(np.cos(u), np.sin(v)) + center[0]
        y = radius * np.outer(np.sin(u), np.sin(v)) + center[1]
        z = radius * np.outer(np.ones(np.size(u)), np.cos(v)) + center[2]
        ax.plot_surface(x, y, z, color=color, alpha=alpha, label=label)
    # ------------------------------------------------

    # --- MODIFIED: Create a layout with 3 plots ---
    fig = plt.figure(figsize=(15, 8))
    ax_3d = fig.add_subplot(1, 2, 1, projection='3d')
    ax_err = fig.add_subplot(2, 2, 2)
    ax_bar = fig.add_subplot(2, 2, 4)
    # ----------------------------------------------------

    # Path tracking performance plot (3D)
    ee_path = np.array(ee_path_log)
    desired_path = np.array(desired_path_log)
    obstacle_path = np.array(obstacle_path_log) 
    
    ax_3d.plot(desired_path[:, 0], desired_path[:, 1], desired_path[:, 2], 'r--', label='Desired Path')
    ax_3d.plot(ee_path[:, 0], ee_path[:, 1], ee_path[:, 2], 'b-', label='Actual Path')
    ax_3d.plot(obstacle_path[:, 0], obstacle_path[:, 1], obstacle_path[:, 2], 'g-', label='Obstacle Path')
    
    plot_sphere(ax_3d, obs_pos_final, obs_radius, 'g', alpha=0.3)
    plot_sphere(ax_3d, obs_pos_final, d_min, 'g', alpha=0.1)
    
    ax_3d.set_xlabel('X [m]'); ax_3d.set_ylabel('Y [m]'); ax_3d.set_zlabel('Z [m]')
    ax_3d.set_title('Path Tracking');
    
    from matplotlib.patches import Patch
    legend_elements = [plt.Line2D([0], [0], color='r', linestyle='--', label='Desired Path'),
                       plt.Line2D([0], [0], color='b', label='Actual Path'),
                       plt.Line2D([0], [0], color='g', label='Obstacle Path'),
                       Patch(facecolor='g', alpha=0.1, label='Safety Boundary')]
    ax_3d.legend(handles=legend_elements)

    # Error convergence plot
    ax_err.plot(times, error_norm, 'm-')
    ax_err.set_xlabel('Time [s]'); ax_err.set_ylabel('||error|| [m]'); ax_err.set_title('Tracking Error')
    ax_err.grid(True)

    # Barrier function plot
    ax_bar.plot(times, barrier_h_log, 'k-')
    ax_bar.axhline(y=0, color='r', linestyle='--', label='Safety Boundary (h=0)')
    ax_bar.set_xlabel('Time [s]'); ax_bar.set_ylabel('h(q)'); ax_bar.set_title('Barrier Function Value')
    ax_bar.legend(); ax_bar.grid(True)
    
    plt.tight_layout()
    plt.show()